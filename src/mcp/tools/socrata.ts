...
export function registerSocrataTools(server: McpServer, config: AppConfig, logger: Logger): void {
    ...
    async (input, extra) => {
      try {
        const structuredContent = await searchSocrataDatasets(input, config, {
          logger: logger.child({ op: "catalog_search" }),
          signal: extra.signal,
        });
    ...
    async (input, extra) => {
      try {
        const structuredContent = await describeSocrataDataset(input, config, {
          logger: logger.child({ op: "dataset_describe" }),
          signal: extra.signal,
        });
    ...
    async (input, extra) => {
      try {
        const structuredContent = await querySocrataDataset(input, config, {
          logger: logger.child({ op: "dataset_query" }),
          signal: extra.signal,
        });
  ...
  server.registerResource(
    ...
    async (uri, variables, extra) => {
      ...
      const result = await describeSocrataDataset({ source_id: sourceId }, config, {
        logger: logger.child({ op: "dataset_metadata_resource" }),
        signal: extra.signal,
      });
