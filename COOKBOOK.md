...
The executor embeds the full plan and runs only when `status` is `ready`; otherwise it returns `execution_status: "blocked"`. `bcn_answer_city_query` uses that same executor, then adds `answer_text`, `answer_markdown`, `answer_type`, compact grouped/nearest-row summaries, warning `caveats`, informational `execution_notes`, selected resource metadata, citation guidance, and the raw `final_result`. Blocked answers include normalized `selection_options` with stable labels, provenance, confidence, and `resume_arguments` for caller pickers. Row summaries include `summary.rows[].source_row` so clients can render the concise label/fields view while preserving the exact row behind it, and row/nearest summaries include `summary.map_points[]` when coordinates are available. `place_kind: "point"` resolves only point-like BCN sources (`landmark` and `facility`), while street/area kinds pass through. Area candidates populate `within_place` from `selected_candidate.area_ref`.
